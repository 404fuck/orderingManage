 ...
 * @LastEditTime: 2021-03-31 16:44:19
 * @LastEditors: Please set LastEditors
 * @Description: 修改职级
 * @FilePath: \sjoyOrderingManage\src\views\employeeManage\editGrade.vue
-->
<template>
    <div class="main">
        ...
        <el-breadcrumb separator="/">
            <el-breadcrumb-item>
                <router-link tag="a" :to="{name: 'employeeManage', params: {obj2:getPropOjb}}" class="crumbsLink">{{ $t("sidebar.yuangong") }}</router-link>
            </el-breadcrumb-item>
            <el-breadcrumb-item>
        ...
        <div class="main-content">
            <el-row style="padding-top: 20px">
                <div class="title-line">{{ $t("grade.update") }}</div>
            </el-row>
            <hr />
                            ...
                            <div class="grade-centent">
                                ...
                                <div class="row-tab-pane">
                                    ...
                                    <el-tabs :tab-position="'left'"   v-show="isActive==1">
                                        ...
                                        >
                                                <div class="check-all">
                                                    <el-checkbox v-model="items.isChecked" @change="handleCheckAllChange(items,items.isChecked)" :disabled="ruleForm.main_receipt==1?true:false">{{$t('all')}}</el-checkbox>
                                                </div>
                                                <el-checkbox 
                                                    v-for="(item,index) in items.role_list" 
                                                    :label="item" 
...
export default {
    ...
    methods:{
        ...
        classDetail(){
            let data = {
                dep_id: this.dep_ID,
                job_level_code: this.$route.query.job_level_code,
            };
            let that = this;
                ...
                if (res.code == "1") {
                    if (res.data != null) {
                        ...
                        that.ruleForm.manager_role_list.forEach(items=>{
                            let isAllChecked = items.role_list.every(item=> item.role_chose =='1'); //判断每一项子类是否全选了
                            if(isAllChecked){
        ...
        //取消
        cancelForm() {
            this.$router.push({name:'employeeManage',params:{obj2:this.getPropOjb}});
        },
         //重置
        resetForm(formName) {
            this.ruleForm.manager_role_list.forEach(items=>{
